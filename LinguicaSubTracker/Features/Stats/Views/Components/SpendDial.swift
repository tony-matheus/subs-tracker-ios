...
struct SpendDial: View {
    ...
    /// Spin the ring into place on first appear as an "I'm draggable" hint.
    var hintRotationOnAppear: Bool = false
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            if hintRotationOnAppear {
                // Start rotated, then spring back so the ring visibly spins
                // and gently overshoots — signalling it can be dragged.
                rotation = -65
                withAnimation(.spring(response: 0.9, dampingFraction: 0.55).delay(0.35)) {
                    rotation = 0
                }
            }
        }
